...
/* Если оценок для этого текста ещё нет (обычно их делает экран загрузки) —
   оцениваем здесь, показывая экран ожидания. */
function scoreProfile(P, id) {
  if (!SC.needsScoring(P)) return Promise.resolve();
  showScoring(true);
  return SC.scoreProfile(P, id).then(function () { showScoring(false); });
}
...
function modelFrom(P) {
  var gpa = parseFloat(String(P.gpa).replace(',', '.'));   // «3,73» тоже 3.73
...
function whyCannotBuild() {
  ...
  if (!(parseFloat(String(P.gpa).replace(',', '.')) > 0))
...
function relayout() {
  ...
  [].slice.call(grid.children).forEach(function (c) {
    ...
    // заголовок группы помечен в groupCards: по тексту стиля нельзя — Safari пишет
    // grid-column как grid-column-start/-end, и заголовок принимался за карточку
    if (c.getAttribute('data-am-head')) {
